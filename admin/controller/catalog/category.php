...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->error['seo_text'])) {
			$data['error_seo_text'] = $this->error['seo_text'];
		} else {
			$data['error_seo_text'] = array();
		}
        ...
        /* MenuIcon */
            if (isset($this->request->post['iconMenu'])) {
                $data['iconMenu'] = trim( $this->request->post['iconMenu']);
            } elseif (!empty($category_info)) {
                $data['iconMenu'] = $category_info['icon_menu'];
            } else {
                $data['iconMenu'] = '';
            }

            if (isset($this->request->post['iconMenu']) && is_file(DIR_IMAGE . $this->request->post['iconMenu'])) {
                $data['iconMenuThumb'] = $this->model_tool_image->resize($this->request->post['iconMenu'], 100, 100);
            } elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['icon_menu'])) {
                $data['iconMenuThumb'] = $this->model_tool_image->resize($category_info['icon_menu'], 100, 100);
            } else {
                $data['iconMenuThumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
            }

            $data['iconMenuPlaceholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
        /* MenuIcon */
		...
		if (isset($this->request->post['seriacheck'])) {
			$data['seriacheck'] = $this->request->post['seriacheck'];
		} elseif (!empty($category_info)) {
			$data['seriacheck'] = $category_info['seriacheck'];
		} else {
			$data['seriacheck'] = false;
		}
