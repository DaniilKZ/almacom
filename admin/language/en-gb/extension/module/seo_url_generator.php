...
$_['text_about_pro'] = '<p>Version <b>SEO URL Generator FREE by Serge Tkach</b> can generate SEO URL not only for a single product, but massively generate CNC for hundreds or thousands of products that are already in the store. Mass generation can also come in handy if your store already has a CNC, but they are outdated (using underscore) or if you need to replace them with a shorter version (for example you can use product_id).</p>';