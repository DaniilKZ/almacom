...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		foreach ($order_products as $order_product) {
			...
			$priceProduct = $this->currency->format($order_product['price'] + ($this->config->get('config_tax') ? $order_product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']);
			...
			$amoCrmProduct .= $order_product['name']." - ".$order_product['model']." x".$order_product['quantity']." = ".$priceProduct."\n";
		...
		if (!$from) {
			...
			$shipping_address = (!empty($data['shipping_address'])) ? $data['shipping_address'] : '';
			$name = ( !empty($data['payment_address']) ) ? $data['payment_address'] : '';

			$tel = (!empty($data['telephone'])) ? $data['telephone'] : '';
			
			$paymentmethod = (!empty($data['payment_method'])) ? $data['payment_method'] : '';
			$dateadded = (!empty($data['date_added'])) ? $data['date_added'] : ''; 
			$numberorder = (!empty($data['title'])) ? $data['title'] : '';

			$amoCrmOrderCheck = true;
			$amoCrmProductTotal  = (!empty($data['totals'][0])) ? $data['totals'][0]['text'] : '';
