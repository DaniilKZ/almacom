...
class ControllerProductCategory extends Controller {
	public function index() {
        ...
        require_once('./system/library/mobiledetect.php');
		...
		if ($category_info) {
			...
			if ($category_id == 20) {
				$data['filter_column_check'] = false;
			}else{
				$data['filter_column_check'] = true;
			}
			...
			/*if (isset( $this->session->data['user_id'] ) && $this->session->data['user_id'] == 1) {
			...
			foreach ($results as $result) {
				...
				// CUSTOM CODE -- PRODUCT PERCENT CALC

				if ((float)$price && (float)$special) {
			...
			if ($result['quantity'] <= 0) { 
				$stock = '<div class="card-product-presence"><i class="sprite sprite-clock"><svg><use xlink:href="#clock"></use></svg></i><span>' . $result['stock_status'] . '</span></div>';
