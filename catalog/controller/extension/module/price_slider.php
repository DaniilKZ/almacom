    ...
    public function index()
    {
        ...
        if (isset($this->request->get['path'])) {
            ...
            if (isset($this->request->get['pr'])) {
                ...
                $pr_value = explode(',', $this->request->get['pr']);

                if (is_array($pr_value) && isset($pr_value) ) {
                    $data['price_range'] = $pr_value;
                }
                
                if (!empty($data['price_range']) && isset($data['price_range'][0]) && $data['price_range'][1] ) {

                    $price_min_value = (is_int((int)$data['price_range'][0]) ) ? (int)$data['price_range'][0] : 0; 
                    $price_max_value = (is_int((int)$data['price_range'][1]) ) ? (int)$data['price_range'][1] : 0; 

                    if ( ($price_min_value >= 1 && $price_max_value > 0) && ( is_numeric($price_min_value) && is_numeric($price_max_value) ) ) {
                        $price_min = $this->currency->convert( $price_min_value, $this->session->data['currency'], $this->config->get('config_currency'));

                        $price_max = $this->currency->convert( $price_max_value, $this->session->data['currency'], $this->config->get('config_currency'));
